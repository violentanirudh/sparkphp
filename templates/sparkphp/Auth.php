...
class Auth
{
    ...
    public function signup($name, $email, $password, $role = 'user', $extra = [])
    {
        ...
        $exists = $this->db->table('users')->where('email = :email', ['email'=>$email])->first();
        ...
        $id = $this->db->table('users')->insert($fields);
        return $id;
    }
    ...
    {
        $user = $this->db->table('users')->where('email = :email', ['email'=>$email])->first();
    ...
    {
        $user = $this->db->table('users')->where('id = :id', ['id'=>$user_id])->first();
        if (!$user || !password_verify($old, $user['password'])) {
            return ['error' => 'Old password incorrect'];
        }
        $hashed = password_hash($new, PASSWORD_BCRYPT);
        $this->db->table('users')->where('id = :id', ['id'=>$user_id])->update(['password' => $hashed]);
        return true;
    }

    // Get or update user profile
    public function profile($user_id, $details = [])
    {
        if (empty($details)) {
            $user = $this->db->table('users')->where('id = :id', ['id' => $user_id])->first();
            ...
            $this->db->table('users')->where('id = :id', ['id' => $user_id])->update($details);
            return true;
        }
    }
    ...
    public function verification($email = null, $token = null)
    {
        ...
        // Validate verification token
            ...
            $user = $this->db->table('users')->where('email = :email', ['email' => $payload['email']])->first();
            if (!$user) {
                return ['error' => 'User not found'];
            }
            $this->db->table('users')->where('email = :email', ['email' => $payload['email']])->update([
                'email_verified' => 1,
                'updated_at' => date('Y-m-d H:i:s')
            ]);
            return ['success' => true];
